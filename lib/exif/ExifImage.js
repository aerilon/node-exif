 ...
 *  - input The image to get Exif data from can be either a filesystem path or
 *          a Buffer.
 *
...
function ExifImage (options, callback)
{
  ...
  this.ee.on('JPEG-APP1', this.processJPEG_APP1.bind(this))
...
ExifImage.prototype.processOpenedFile = function (err, fd) {
  ...
  fs.fstat(this._file.fd, this.processFileStats.bind(this))
}

ExifImage.prototype.processFileStats = function (err, stats)
{
  ...
  this._file.buf = new Buffer(stats.size)
  this._file.offset = 0

  this.ee.emit('fill-buffer')
}

ExifImage.prototype.fillBuffer = function ()
{
  var defaultReadSize, readSize
  var file = this._file

  defaultReadSize = 64 * 1024
  readSize = Math.min(defaultReadSize, file.buf.length - file.buf.offset)

  fs.read(file.fd, file.buf, file.offset, readSize, null, this.processFileRead.bind(this))
}

ExifImage.prototype.processFileRead = function (err, len, buf)
{
  ...
  this._file.offset += len

  this.processBuffer(buf)
}


ExifImage.prototype.validateBuffer = function (offset, len)
{
  var file = this._file

  if (typeof file == "undefined") {
    this.callback(new Error("Unexpected end of buffer"))
    return false
  }

  if (offset + 2 > file.offset) {
    this.ee.emit('fill-buffer')
    return false
  }
...
ExifImage.prototype.processJPEGImage = function (data, offset)
{

  if (this.JPEGSegments.length != 0) {
    var last_segment = this.JPEGSegments[this.JPEGSegments.length - 1]
  ...
  while (offset < data.length) {
    ...
    if (!this.validateBuffer(offset, segmentLength))
      return

    this.JPEGSegments.push({
      name: name,
      headerLength: (segment.len == 0) ? 0 : 2,
      start: offset - 2,
  ...
  this.JPEGSegments.forEach(function(segment) {
    var data = this._file.buf.slice(segment.start, segment.end)
    this.ee.emit("JPEG-" + segment.name, data)
  }, this)
}
	
ExifImage.prototype.processJPEG_APP1 = function (data)
{
  var offset = 4
  ...
  this.callback(null, this.exifData)
}
...
ExifImage.prototype.extractExifData = function (data)
{
   ...
   * in little endian format or 0x4D4D ("MM") if stored in big endian format
   */
  var identification = data.readUInt16(0)
    if (identification == 0x4949) {
      data.setEndian(data.Endian.LITTLE)
  ...
  /*
   * EXIF IFD -
   * Look for a pointer to the Exif IFD in IFD0 and extract information from it
   * if available
   */
  ...
  /*
   * GPS IFD -
   * Look for a pointer to the GPS IFD in IFD0 and extract information from it
   * if available
   */
...
ExifImage.prototype.extractExifEntry = function (data, entryOffset, tags)
{
  var field, offset, type

  field = {
    tag: {
      ID: data.readUInt16(entryOffset),
      name: null
    },
    type: data.readUInt16(entryOffset + 2),
    count: data.readUInt32(entryOffset + 4),
    value: []
  }

  // The tag ID may correspond to more then one name, so check which
  if (tags && tags[field.tag.ID]) {
  ...
  if (field.type == 2) {
    if (field.value[field.value.length - 1] != 0)
      return null
    field.value.pop()
    field.value = String.fromCharCode.apply(null, field.value)
  }

  if (field.type == 5 || field.type == 10) {
    var val = []
    for (var i = 0; i < field.value.length; i+=2) {
      if (field.value[i + 1] == 0)
         return null
//    val[i/2] =  field.value[i] / field.value[i + 1] + " (" + field.value[0] + "/" + field.value[1] + ")"
      val[i/2] =  field.value[i] / field.value[i + 1]
    }
    field.value = val
  }
...
ExifImage.TIFFTYPES =
{
  1 : { len: 1, count: 1, signed: false },
  2 : { len: 1, count: 1, signed: false },
  3 : { len: 2, count: 1, signed: false },
  4 : { len: 4, count: 1, signed: false },
  5 : { len: 4, count: 2, signed: false },
  6 : { len: 1, count: 1, signed: true },
  7 : { len: 1, count: 1, signed: false },
  8 : { len: 2, count: 1, signed: true },
  9 : { len: 4, count: 1, signed: true },
  10 : { len: 4, count: 2, signed: true },
}

ExifImage.MAKERNOTES =
[
  { prefix: "OLYMP\x00\x01",  library: "./makernotes/olympus" },
  { prefix: "OLYMP\x00\x02",  library: "./makernotes/olympus" },
  { prefix: "AGFA",           library: "./makernotes/agfa" },
  { prefix: "EPSON\x00\x01\x00",  library: "./makernotes/epson" },
  { prefix: "FUJIFILM",       library: "./makernotes/fujifilm" },
  { prefix: "Panasonic",      library: "./makernotes/panasonic" },
  { prefix: "SANYO",          library: "./makernotes/sanyo" },
]
