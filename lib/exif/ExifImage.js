...
ExifImage.prototype.processImage = function (data, callback) {
  ...
  try {
    switch (self.imageType) {
      case "JPEG":
        self.processJPEGImage(data, offset, callback)
        break;
      default:
...
ExifImage.prototype.processJPEGImage = function (data, offset, callback) {
  ...
  while (offset < data.length) {
    ...
    this.ee.emit("JPEG-" + name, data.slice(offset + 2, offset + segmentLength), callback)
...
ExifImage.prototype.processJPEG_APP1 = function (data, callback) {

  var self = this;

  if (!data.startWithString('Exif\0\0', 0))
    return;

  self.extractExifData(data.slice(6, data.length));
...
ExifImage.prototype.extractExifData = function (data) {
  ...
  var versionOffset = 2
  var version  = data.readUInt16(versionOffset, this.isBigEndian)
  if (version != 42) {
  ...
  /********************************* IFD0 **********************************/
  ifdOffset = data.readUInt32(4, this.isBigEndian)
  ...
  var nextIfdOffset = data.readUInt32(ifdOffset + 2 + (numberOfEntries * 12), this.isBigEndian)
...
ExifImage.prototype.extractExifIFDEntries = function (data, offset, store, tags) {
  ...
  var self = this
  var entryCount = data.readUInt16(offset, this.isBigEndian);

  for (var i = 0; i < entryCount; i++) {
    var start = offset + 2 + (i * 12)
    var entry = self.extractExifEntry(data, start, this.isBigEndian, tags);
...
ExifImage.prototype.extractExifEntry = function (data, entryOffset, isBigEndian, tags) {
  ...
  var field = {
    tag: {
      ID: data.readUInt16(entryOffset, isBigEndian),
      name: null
    },
    type: data.readUInt16(entryOffset + 2, isBigEndian),
    count: data.readUInt32(entryOffset + 4, isBigEndian),
    value: []
  }
  ...
  if (field.count > (4 / type.size))
    offset = data.readUInt32(entryOffset + 8, isBigEndian);
  ...
  for (i = 0; i < field.count; i++) {
    for (j = 0; j < type.count; j++) {
      var val = (type.signed) ? data.readInt(offset, type.len) :
        data.readUInt(offset, type.len, isBigEndian);
