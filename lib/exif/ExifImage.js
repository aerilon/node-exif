...
ExifImage.prototype.processImage = function (data, callback) {
  ...
  var soiHdr = data.getShort(offset, true)
  if (soiHdr == 0xFFD8) {
    self.imageType = 'JPEG';
  }
  ...
  switch (self.imageType) {
    case "JPEG":
      self.processJPEGImage(data, offset + 2, callback)
      break;
    default:
...
ExifImage.prototype.processJPEGImage = function (data, offset, callback) {
  ...
  try {
    while (offset < data.length) {
      var segmentHdr = data.getShort(offset, true)
      var segmentLength = data.getShort(offset + 2, true) - 2;

      offset += 4

      if (segmentHdr != 0xFFE1) {
      ...
      self.extractExifData(data.slice(offset + 6, offset + segmentLength));
...
ExifImage.prototype.extractExifData = function (data) {
  ...
  var identification = data.getShort(0)
  ...
  var versionOffset = 2
  var version  = data.getShort(versionOffset, this.isBigEndian)
  if (version != 42) {
  ...
  // the data from
  this.parseIFD = function (data, offset, store, tags) {
    var self = this
    var numberOfEntries = data.getShort(offset, this.isBigEndian);

    for (var i = 0; i < numberOfEntries; i++) {
      var entryStart = offset + 2 + (i * 12)
      var exifEntry = self.extractExifEntry(data, entryStart, this.isBigEndian, tags);
      if (exifEntry && exifEntry.tagName !== null)
        store[exifEntry.tagName] = exifEntry.value;
    }
  ...
  /********************************* IFD0 **********************************/
  ifdOffset = data.getLong(4, this.isBigEndian)
  numberOfEntries = this.parseIFD(data, ifdOffset, this.exifData.image, ExifImage.TAGS.exif);
  ...
  if (nextIfdOffset != 0x00000000)
    this.parseIFD(data, nextIfdOffset, this.exifData.thumbnail, ExifImage.TAGS.exif);
  ...
  if (typeof subIfdOffset != "undefined")
    this.parseIFD(data, subIfdOffset, this.exifData.exif, ExifImage.TAGS.exif);
  ...
  if (typeof subIfdOffset != "undefined")
    this.parseIFD(data, subIfdOffset, this.exifData.gps, ExifImage.TAGS.gps);
  ...
  if (typeof subIfdOffset != "undefined")
    this.parseIFD(data, subIfdOffset, this.exifData.interoperability, ExifImage.TAGS.exif);
  ...
  var makernoteIfdData = this.exifData.exif[ExifImage.TAGS.exif[0x927C]]
  if (typeof makernoteIfdData != "undefined") {

    for (var i = 0; i < ExifImage.MAKERNOTES.length; i++) {
      var makernote = ExifImage.MAKERNOTES[i];
      if (makernoteIfdData.startWithString(makernote.prefix, 0)) {
    ...
    if (typeof this.extractMakernotes != "undefined") {
      this.exifData.makernote = this.extractMakernotes(data.slice(self.makernoteOffset, data.length));
    } else {
...
ExifImage.prototype.extractExifEntry = function (data, entryOffset, isBigEndian, tags) {
  ...
  var entry = {
    tagId : data.getShort(entryOffset, isBigEndian),
    tagName : null,
  ...
  switch (entry.format) {
    ...
    case 0x0001: // unsigned byte, 1 byte per component
      entry.valueOffset = (entry.components <= 4) ? entryOffset + 8 : data.getLong(entryOffset + 8, isBigEndian);
    ...
    case 0x0003: // unsigned short, 2 byte per component
      entry.valueOffset = (entry.components <= 2) ? entryOffset + 8 : data.getLong(entryOffset + 8, isBigEndian);
      for (var i = 0; i < entry.components; i++)
        entry.value.push(data.getShort(entry.valueOffset + i * 2, isBigEndian));
      break;

    case 0x0004: // unsigned long, 4 byte per component
      entry.valueOffset = (entry.components == 1) ? entryOffset + 8 : data.getLong(entryOffset + 8, isBigEndian);
      for (var i = 0; i < entry.components; i++)
    ...
    case 0x0005: // unsigned rational, 8 byte per component (4 byte numerator and 4 byte denominator)
      entry.valueOffset = data.getLong(entryOffset + 8, isBigEndian);
      for (var i = 0; i < entry.components; i++)
