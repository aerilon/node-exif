...
ExifImage.prototype.extractExifData = function (data) {
  ...
  var identification = data.getShort(tiffOffset)
    if (identification == 0x4949) {
      this.isBigEndian = false;
    } else if (identification == 0x4D4D) {
      this.isBigEndian = true;
  } else {
    throw new Error('Invalid TIFF data! Expected 0x4949 or 0x4D4D at offset '+(tiffOffset)+' but found 0x'+identification.toHex()+".");
  }

  // Valid TIFF headers always have 0x002A here
  var versionOffset = tiffOffset + 2
  var version  = data.getShort(versionOffset, this.isBigEndian)
  if (version != 42) {
    var expected = (this.isBigEndian) ? '0x002A' : '0x2A00';
    throw new Error('Invalid TIFF data! Expected 42 at offset '+ versionOffset +' but found 0x'+version.toHex()+".");
  }

  // Each IFD entry consists of 12 bytes which we loop through and extract
  // the data from
  this.parseIFD = function (data, offset, tiffOffset, store, tags) {
    var self = this
    var numberOfEntries = data.getShort(offset, this.isBigEndian);

    for (var i = 0; i < numberOfEntries; i++) {
      var entryStart = offset + 2 + (i * 12)
      var exifEntry = self.extractExifEntry(data, entryStart, tiffOffset, this.isBigEndian, tags);
  ...
  numberOfEntries = this.parseIFD(data, ifdOffset, tiffOffset, this.exifData.image, ExifImage.TAGS.exif);
  ...
  if (nextIfdOffset != 0x00000000) {
    ...
    this.parseIFD(data, ifdOffset, tiffOffset, this.exifData.thumbnail, ExifImage.TAGS.exif);
  }
  ...
  if (typeof subIfdOffset != "undefined") {
    ...
    ifdOffset = tiffOffset + subIfdOffset;
    this.parseIFD(data, ifdOffset, tiffOffset, this.exifData.exif, ExifImage.TAGS.exif);
  }
  ...
  if (typeof subIfdOffset != "undefined") {
    ...
    ifdOffset = tiffOffset + subIfdOffset;
    this.parseIFD(data, ifdOffset, tiffOffset, this.exifData.gps, ExifImage.TAGS.gps);
  }
  ...
  if (typeof subIfdOffset != "undefined") {
    ...
    ifdOffset = tiffOffset + subIfdOffset;
    this.parseIFD(data, ifdOffset, tiffOffset, this.exifData.interoperability, ExifImage.TAGS.exif);
  }
