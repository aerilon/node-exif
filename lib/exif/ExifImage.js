...
ExifImage.prototype.extractExifIFDEntries = function (data, offset, store, tags) {
  ...
  var self = this
  var entryCount = data.getShort(offset, this.isBigEndian);
...
ExifImage.prototype.extractExifEntry = function (data, entryOffset, isBigEndian, tags) {
  ...
  var field = {
    tag: {
      ID: data.getShort(entryOffset, isBigEndian),
      name: null
    },
    type: data.getShort(entryOffset + 2, isBigEndian),
    count: data.getLong(entryOffset + 4, isBigEndian),
    value: []
  }
  ...
  if (field.count > (4 / type.size))
    offset = data.getLong(entryOffset + 8, isBigEndian);
