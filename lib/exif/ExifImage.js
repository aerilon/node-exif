...
ExifImage.prototype.loadImage = function (image) {
  ...
  if (image.constructor.name === 'Buffer') {
    ...
    fs.readFile(image, function (error, data) {
      if (error)
        this.callback(new Error('Encountered the following error while trying to read given image: '+error));
      else
...
ExifImage.prototype.processImage = function (data) {
...
ExifImage.prototype.processJPEGImage = function (data, offset) {
  ...
  while (offset < data.length) {
    ...
    this.ee.emit("JPEG-" + name, data.slice(offset + 2, offset + segmentLength))
...
ExifImage.prototype.processJPEG_APP1 = function (data) {
  ...
  try {
    this.extractExifData(data.slice(6, data.length));
  } catch (error) {
    this.callback(error)
    return
  }
